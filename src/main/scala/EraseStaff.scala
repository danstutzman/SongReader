...
object EraseStaff {
  ...
  def run(input:GrayImage, staff:Staff, eraseBackground:Boolean,
      staffName:String) = {
    val (_, _, whereNotesAre) = separateNotes(input, staffName)
    ...
    demo.saveTo(new File("demos/erase.%s.png".format(staffName)))
    ...
    demo2.saveTo(new File("demos/erase2.%s.png".format(staffName)))
    demo4.saveTo(new File("demos/erase4.%s.png".format(staffName)))
    ...
    if (eraseBackground) demo2 else demo4
  }
}
